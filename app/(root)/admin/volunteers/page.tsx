...
export default function VolunteersPage() {
  ...
  const handleEdit = (volunteer: VolunteerRegistration) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/volunteer-registration?id=${deleteVolunteerId}`, {
        method: 'DELETE'
      })
    ...
    <div className="container mx-auto py-8">
      ...
      <Card className="p-6">
        {volunteers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-lg text-grey-600">暂无义工申请</p>
          </div>
        ) : (
      ...
      <Dialog open={!!editingVolunteer} onOpenChange={() => setEditingVolunteer(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>编辑义工申请</DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              >
                <option value="twice">每月 2 次</option>
                <option value="once">每月 1 次</option>
