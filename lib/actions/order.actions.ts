...
export const getOrdersByEvent = unstable_cache(
  async ({ searchString, eventId, select }: GetOrdersByEventParams) => {
    try {
      await connectToDatabase();
  ...
  ['orders-by-event'], // Cache key prefix
  {
    revalidate: 60, // Cache for 60 seconds
    tags: ['orders', 'event-lookup'] // Tags for manual invalidation if needed
  }
);
...
export const getOrdersByPhoneNumber = unstable_cache(
  ...
  ['orders-by-phone'], // Cache key prefix
  {
    revalidate: 60, // Cache for 60 seconds
    tags: ['orders', 'phone-lookup'] // Tags for manual invalidation if needed
  }
);
